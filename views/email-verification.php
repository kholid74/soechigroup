<?php
  ...
  if($_SESSION['type'] == 'shipping') {

      $sql  = 'SELECT a.*,b.name FROM sch_job_shipping a JOIN sch_master_crewrank b ON a.id_job_name=b.id WHERE a.id="'.$_SESSION['idJob'].'"';
      $dataJob = $object->fetch($sql);

  } elseif ($_SESSION['type'] == 'office'){

    $sql  = 'SELECT * FROM sch_job_office WHERE id="'.$_SESSION['idJob'].'"';
    $dataJob = $object->fetch($sql);
  ...
  if (isset($_POST['submit'])) {
    ...
    if ($email == "") {
      ...
      $failcand     = 'SELECT a.email,b.status FROM sch_candidate_shipping a JOIN sch_cand_shipping_status b ON a.candidate_code=b.candidate_code WHERE b.status="REJECTED" OR b.status="INTERVIEW_FAIL" AND a.email="$email"';
      $showfailcand = $object->fetch($failcand); //cek email sudah terdaftar apa belum
      ...
      if (count($count) > 0) {
        ...
        if ($mail->send()) {
            echo "<script> window.location.assign('".$object->base_path()."verify-code');</script>";
