			...
			<div class="animated fadeIn">
				...
				<div class="card-body">
                    ...
                    <div align="center">
	                    ...
	                    <a href="<?php echo $object->base_path()?>shipping-candidate-shortlisted" class="btn btn-outline-secondary active">
							...
							<table class="table table-responsive-sm table-bordered table-striped table-sm" id="example">
								...
								<tbody>
									...
									if(isset($_POST['_filterCand'])){
								    	...
								    	$sql = "SELECT a.*,b.id_job_name,c.name,d.status FROM sch_candidate_shipping a JOIN sch_job_shipping b ON a.id_job = b.id JOIN sch_master_crewrank c ON b.id_job_name=c.id JOIN sch_cand_shipping_status d ON a.candidate_code=d.candidate_code WHERE d.status='SHORTLISTED' AND c.name='$jobVac'";
									...
									<tr>
										...
										<td><?= $cand['email'] ?></td>
										<td align="center">
											<span class='badge badge-success'><?= $cand['status'] ?></span>
										</td>
										<td align="center">
											<a class="btn btn-success btn-sm" href="<?php echo $object->base_path()?>shipping-candidate-shortlisted-details/<?= $cand['id'] ?>" title="view <?= $cand['first_name'] ?> <?= $cand['last_name'] ?>">
