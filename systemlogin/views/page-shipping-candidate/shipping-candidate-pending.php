		...
		<div id="ui-view" style="opacity: 1;">
			<div class="animated fadeIn">
				...
				<div class="card-body">
                        ...
                        ?>
                    <div align="center">
                    	<a href="<?php echo $object->base_path()?>shipping-candidate-pending" class="btn btn-outline-secondary active">
	                          ...
	                          <label>Filter by :&nbsp;</label>
	                            <select class="form-control" name="_jobvac" style="border-radius: 0px;">
	                              ...
	                              <?php 
	                              	...
	                              	$sql = "SELECT a.*,b.id_job_name,c.name,d.status FROM sch_candidate_shipping a JOIN sch_job_shipping b ON a.id_job = b.id JOIN sch_master_crewrank c ON b.id_job_name=c.id JOIN sch_cand_shipping_status d ON a.candidate_code=d.candidate_code WHERE d.status='PENDING_REVIEW'";
								...
								<tbody>
									<?php
										...
										if(isset($_POST['_filterCand'])){
									    	...
									    	$sql = "SELECT a.*,b.id_job_name,c.name,d.status FROM sch_candidate_shipping a JOIN sch_job_shipping b ON a.id_job = b.id JOIN sch_master_crewrank c ON b.id_job_name=c.id JOIN sch_cand_shipping_status d ON a.candidate_code=d.candidate_code WHERE d.status='PENDING_REVIEW' AND c.name='$jobVac'";
									...
									<tr>
										...
										<td><?= $cand['created'] ?></td>
										<td align="center">
											<a class="btn btn-success btn-sm" href="<?php echo $object->base_path()?>shipping-candidate-pending-details/<?= $cand['id'] ?>" title="View <?= $cand['first_name'] ?> <?= $cand['last_name'] ?>">
