<?php
	...
	$sql = "SELECT a.*,b.id_job_name,c.name,c.deck_engine,c.short_name,c.category,d.status FROM sch_candidate_shipping a JOIN sch_job_shipping b ON a.id_job = b.id JOIN sch_master_crewrank c ON b.id_job_name=c.id JOIN sch_cand_shipping_status d ON a.candidate_code=d.candidate_code WHERE a.id=".$_GET['ids']."";
    ...
    $boc  = 'SELECT value FROM sch_settings WHERE section="email_boc"';
    $showboc = $object->fetch($boc); 
    ...
    if(isset($_POST['_saveRemarks'])) {
    	...
    	echo "<script> window.location.assign('".$object->base_path()."shipping-candidate-shortlisted-details/".$candidate['id']."'); </script>";
  }
	...
	if(isset($_POST['_sendboc'])){

		$url = ''.BASE_URL.'media/files/'.$candidate['cv'];
		$dataSubject = $candidate['first_name'].' '.$candidate['last_name'].' ('.$candidate['short_name'].')';
		$nameCV = $candidate['first_name'].' '.$candidate['last_name'].'';
	    ...
	    $message = str_replace("%candidate['coc_number']%", $candidate['coc_number'], $message);
	    ...
	    //Content
	    $mail->isHTML(true);              
	    $mail->Subject = '[no-reply] Seeking Approval BOC of '.$dataSubject.'';
	    $mail->MsgHTML($message);

	    $mail->addStringAttachment(file_get_contents($url), 'CV_'.$nameCV.'.pdf');
...
<!-- Main content -->
    <main class="main">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="<?php echo $object->base_path()?>shipping-candidate-shortlisted">Online Test Candidate</a></li>
			...
			<div class="animated fadeIn">
				...
				<div class="card-body">
					...
					<!-- Main row -->
				      <div class="row">
				        <div class="col-lg-12">
				        <a href="<?php echo $object->base_path()?>shipping-candidate-shortlisted" style="font-weight: bold;color: #084f93;"><i class="fa fa-chevron-left" aria-hidden="true"></i>&nbsp;&nbsp;BACK TO APPLICANT LIST</a>
				        <br><br>
						...
						<div align="center">
							...
							<form role="form" method="POST" enctype="multipart/form-data">
								...
								<?php if($candidate['boc_send'] == 0){ ?>

								<input type="submit" name="_sendboc" class="btn btn-flat btn-success" title="Send Email to BOC" onclick="return confirm('Send email to BOC for this candidate ?');" value="SEND EMAIL TO BOC" />
								
								<?php }elseif($candidate['boc_send'] == 1){ ?>

								<input type="submit"  class="btn btn-flat btn-success" title="This candidate has been sent to BOC"  value="SEND EMAIL TO BOC" disabled/>
