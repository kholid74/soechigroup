<?php
   ...
   include '../systemlogin/assets/plugin/phpqrcode/qrlib.php'; 
	...
	$user     = "SELECT * FROM sch_candidate_user WHERE candidate_code='$canCode'";
	$showUser = $object->fetch($user);

	$schedule    = "SELECT * FROM sch_interview_schedule WHERE id_candidate=".$_GET['ids']." AND status='0'";
	$shwschedule = $object->fetch($schedule); 
    ...
    if(isset($_POST['_saveRemarks'])) {
    	...
    	echo "<script> window.location.assign('".$object->base_path()."s-interview-schedule-pending-details/".$candidate['id']."'); </script>";
  }
	...
	if(isset($_POST['_approve'])){
	    ...
	    $fileName = 'cand_'.md5($candidate['candidate_code']).'.png'; 
	     
	    $pngAbsoluteFilePath = $tempDir.$fileName; 
	    $urlRelativeFilePath = '../media/images/qrcode_generate/'.$fileName; 
	    
	    if (!file_exists($pngAbsoluteFilePath)) { 
	        QRcode::png($candidate['candidate_code'], $pngAbsoluteFilePath,"H",8,2); 
	    } else { 
	    }
        ...
        $message = str_replace("%candidate['name']%", $candidate['name'], $message);
        $message = str_replace("%shwschedule['date']%", $shwschedule['date'], $message);
        $message = str_replace("%shwschedule['time']%", $shwschedule['time'], $message);
        $message = str_replace("%authadmin['name']%", $authadmin['name'], $message);
        ...
        echo "<script> window.location('".$object->base_path()."s-interview-schedule-pending'); </script>";
    }
    ...
    if(isset($_POST['_decline'])){
        ...
        $conditions = array('id' =>strip_tags($_POST['id']));
        $statusMsg =  $object->updatedata($namatable,$data,$conditions);
        $message = file_get_contents(''.BASE_URL.'emailtemplates/shipping-reject-interview.html');
        $message = str_replace("%candidate['first_name']%", $candidate['first_name'], $message);
        $message = str_replace("%candidate['email']%", $candidate['email'], $message);
	    $message = str_replace("%showUser['show_password']%", $showUser['show_password'], $message);
	    $message = str_replace("%BASE_URL%", BASE_URL, $message);
...
<!-- Main content -->
    <main class="main">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="<?php echo $object->base_path()?>s-interview-schedule-pending">Interview Candidate</a></li>
			...
			<div class="animated fadeIn">
				...
				<div class="card-body">
					...
					<!-- Main row -->
				      <div class="row">
				        <div class="col-lg-12">
				        <a href="<?php echo $object->base_path()?>s-interview-schedule-pending" style="font-weight: bold;color: #084f93;"><i class="fa fa-chevron-left" aria-hidden="true"></i>&nbsp;&nbsp;BACK TO APPLICANT LIST</a>
				        <br><br>
						...
						<br>
				          <div align="center">
							<form role="form" method="POST" enctype="multipart/form-data">
								<input type="hidden" name="id" value="<?= $shwschedule['id'] ?>">
								
								<input type="submit" name="_approve"  class="btn btn-flat btn-primary" title="Approve Schedule" value="APPROVE SCHEDULE" onclick="return confirm('Approve Interview Schedule ?');"/>
								 <input type="submit" name="_decline"  class="btn btn-flat btn-danger" title="Reject Schedule" value="REJECT SCHEDULE" onclick="return confirm('Reject Interview Schedule ?');"/>
							</form>
