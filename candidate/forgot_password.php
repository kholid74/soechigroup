...
if (isset($_POST['submit'])) {
    ...
    if ($email == "") {
      ...
      $sql = "SELECT * FROM sch_candidate_user WHERE email='$email'";
      ...
      if (count($count) > 0) {
        ...
        $verifyEmail = "UPDATE `sch_candidate_user` SET `token`='".$token."' WHERE `email`='".$email."'";

        $object->add($verifyEmail);

        $message = file_get_contents('../emailtemplates/forgotpass_candidate.html');
